...
train_features, test_features, train_labels, test_labels = (
    train_test_split(preprocessed_features, labels, test_size=1/3))
...
class MLP(nn.Module):
    def __init__(self,
                 input_size: int,
                 hidden_layer_size_p: int,
                 output_size: int,
                 activation_fn: Callable[[torch.Tensor], torch.Tensor] = F.relu):
        super().__init__()
        self.l1 = nn.Linear(input_size, hidden_layer_size_p)
        self.l2 = nn.Linear(hidden_layer_size_p, output_size)
...
def accuracy(probs: torch.FloatTensor, targets: torch.LongTensor) -> float:
    ...
    # Find the maximum value along dimension 1 (the second dimension)
    forcast_types = torch.argmax(probs, dim=1)

    # Reshape the max_values tensor to have a size of (80, 1)

    matchingNum = torch.sum(forcast_types == targets).item()

    return matchingNum / probs.shape[0]
